...
look_cor <- function(all_id, dat_cor, respvar, ord){
  ...
  modall <- lm(as.formula(paste(respvar," ~ ", paste(all_id, collapse= "+"))), data = dat_cor, na.action = na.omit) 
  ...
  for (i in 1:length(all_id)){
    ...
    modminus              <- lm(as.formula(paste(respvar," ~ ", paste(all_id[-i], collapse= "+"))), data = dat_cor, 
                                na.action = na.omit) 
    # Rsquared of the lm between "respvar" and variable i
    modalone              <- lm(as.formula(paste(respvar," ~ ", all_id[i])), data = dat_cor, na.action = na.omit) 
...
reduce_mod <- function(sp_list, respvar, dat_cor, thr){
  ...
  mod_full_sum_coeff_sorted <- mod_full_sum$coefficients[order(mod_full_sum$coefficients[,"Pr(>|t|)"], decreasing = TRUE),]
  mod_full_sum_coeff_sorted <- mod_full_sum_coeff_sorted[!(rownames(mod_full_sum_coeff_sorted) %in% c("sr","I(sr^2)","(Intercept)")),]
...
##                  for div = PD, colnames = id_name of the species, for div = TD or FD colnames(data) = id_code of the species
#' @param tree      phylotree generated by ape::read_tree. Default is NULL. Only needed if div = PD
#' @param dij       distance matrix generated by ade4::dist.kab (Pavoine et al. 2009). Default is NULL. Only needed if div = FD
#' @param tau       threshold for dij (Chao et al. 2019). Default is NULL, recommended value is mean(dij). Only needed if div = FD
...
#' @export
standefsize <- function(div, data, tree = NULL, dij = NULL, tau = NULL, q, hilldiv, log_trans = FALSE, nb = 10, mcores = 20){
  ...
  ind <- do.call(cbind, parallel::mclapply(1:dim(sm)[3], function(i){ 
    ...
    abondquad  <- as.matrix(apply(mat, 1, sum))    # count the number of indiv per quadrat considering all species
    mabondquad <- t(rep.row(abondquad, ncol(mat))) # create a matrix of same size to be able to divide the number of individual of each species per the number of individuals in the entire quadrat
    relab_quad <- mat/mabondquad                   # Relative abundance of each species for all quadrats 
  ...
  # case where the hill number equals 1 and sigma is null create Nan (no division by 0) but if mu is the same than 
  # observed index, SES is null
