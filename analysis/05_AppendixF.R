...
S        <- apply(relab, 1, sum)
if(length(which(S == 0)) != 0){ 
  relab <- relab[- which(rownames(relab) %in% names(which(S==0))),]}

# Same quadrats
ifelse(length(setdiff(as.character(esth_all$quadrat_code), rownames(relab))) != 0,
       esth <- esth_all[-which(esth_all$quadrat_code %in% setdiff(esth_all$quadrat_code,
                                                                  rownames(relab))),],
       esth <- esth_all) # eo ifelse
if(length(setdiff(rownames(relab), esth_all$quadrat_code)) != 0){
  relab <- relab[-which(rownames(relab) %in% setdiff(rownames(relab),
                                                     as.character(esth_all$quadrat_code))),]
} # eo if
...
start_time <- Sys.time()
res        <- find_bestq(div = "TD", data = relab, esth = esth, q_min = -1, q_max = 1, q_nb = 1000,
...
# normalize 
trait_norm <- data.frame(id_name = trait_all[,1],
                         rbind(apply(trait_all[,-c(1:2)], 2, function(col){
  col <- as.numeric(as.character(col))
  normalize(col)
})))

# create objects for dist.ktab function
single <- c("colonial", "gregarious", "unit_height","base_cover", "base_type", "consistency",
            "condition_of_food", "size_of_food", "engineering","coralligenous_builder", 
            "thermal_preference", "sedimentation", "organic_pollution", "salinity", 
            "hydrodynamism", "light", "nb_interactions", "nb_substrate", "nb_def_strat")
...
dis_traits_cora <- as.matrix(ade4::dist.ktab(x = ade4::ktab.list.df(list(
  condition_of_food,
  gregarious, base_type, consistency, size_of_food, engineering, thermal_preference, sedimentation,
  organic_pollution, hydrodynamism, light, unit_height,base_cover, nb_interactions, nb_def_strat,
  nb_substrate, coralligenous_builder, colonial, salinity)),
...
rm(base_cover, base_type, colonial, condition_of_food, consistency, coralligenous_builder, 
   engineering, gregarious, hydrodynamism, light, nb_def_strat, nb_interactions, nb_substrate,
   organic_pollution, salinity, sedimentation, size_of_food, thermal_preference, unit_height,
   i,j,objname, single, trait_all, trait_norm)
...
data <- relab
for (i in 1:ncol(data)){colnames(data)[i] <- as.character(phylo[
  which(phylo$ID_code == colnames(data)[i]),"species"])}
...
start_time <- Sys.time()
res        <- find_bestq(div = "PD", data = data, tree = tree, esth = esth, q_min = -1, q_max = 1,
                         q_nb = 1000, 
                         ncores = nproc, log_trans = log_trans)
end_time   <- Sys.time()
end_time - start_time
...
for (i in 1:ncol(abund_names)){
  colnames(abund_names)[i] <- as.character(phylo[
    which(as.character(phylo$ID_code) == colnames(abund_names)[i]),"species"])}
...
start_time <- Sys.time()
qFD_all    <- standefsize(div = "FD", data = abund, dij = dis_traits_cora, tau = tau,
                          q = bestq_FD, hilldiv = qFD, log_trans = log_trans,
                          nb = 1000, mcores = nproc) 
...
file.remove(hh("output", "05_hillnb.RData"), hh("output", "05_funct_dist_matrix.RData"), 
            hh("output", "05_phylo_tree.RData"))
