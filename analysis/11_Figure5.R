...
# aes_rich    <- df[which(df$quadrat_code %in% c("aes_rich1", "aes_rich2", "aes_rich3", "aes_rich4")),]
# aes_p       <- df[which(df$quadrat_code %in% c("aes_poor1", "aes_poor2" ,"aes_poor3", "aes_poor4" )),]
# naes_p      <- df[which(df$quadrat_code %in% c("naes_poor1", "naes_poor2", "naes_poor3", "naes_poor4")),]
# naes_rich   <- df[which(df$quadrat_code %in% c("naes_rich1", "naes_rich1", "naes_rich1", "naes_rich1")),]
#
...
#' @export
format_pict <- function(path_ori, name_ori, path_final, name_final, point_type, fill_col, border_col){
  ...
  plot(img, axes = FALSE)
  points(x = 0.1*imager::width(img), y = 0.1*imager::height(img), pch = pch, col = border_col, bg = fill_col, cex = size )
  dev.off()
...
# aes_p1 <- format_pict(path_ori = path_all_picts, path_final = path_aesp, name_ori = paste0(corner_pict$aes_p[1], ".png"), name_final = "aes_p1.pdf", point_type = "square", fill_col = fill_col, border_col = border_col)
# aes_p2 <- format_pict(path_ori = path_all_picts, path_final = path_aesp, name_ori = paste0(corner_pict$aes_p[2], ".png"), name_final = "aes_p2.pdf", point_type = "square", fill_col = fill_col, border_col = border_col)
# aes_p3 <- format_pict(path_ori = path_all_picts, path_final = path_aesp, name_ori = paste0(corner_pict$aes_p[3], ".png"), name_final = "aes_p3.pdf", point_type = "square", fill_col = fill_col, border_col = border_col)
# aes_p4 <- format_pict(path_ori = path_all_picts, path_final = path_aesp, name_ori = paste0(corner_pict$aes_p[4], ".png"), name_final = "aes_p4.pdf", point_type = "square", fill_col = fill_col, border_col = border_col)
#
# naes_p1 <- format_pict(path_ori = path_all_picts, path_final = path_naesp, name_ori = paste0(corner_pict$naes_p[1], ".png"), name_final = "naes_p1.pdf", point_type = "circle", fill_col = fill_col, border_col = border_col)
# naes_p2 <- format_pict(path_ori = path_all_picts, path_final = path_naesp, name_ori = paste0(corner_pict$naes_p[2], ".png"), name_final = "naes_p2.pdf", point_type = "circle", fill_col = fill_col, border_col = border_col)
# naes_p3 <- format_pict(path_ori = path_all_picts, path_final = path_naesp, name_ori = paste0(corner_pict$naes_p[3], ".png"), name_final = "naes_p3.pdf", point_type = "circle", fill_col = fill_col, border_col = border_col)
# naes_p4 <- format_pict(path_ori = path_all_picts, path_final = path_naesp, name_ori = paste0(corner_pict$naes_p[4], ".png"), name_final = "naes_p4.pdf", point_type = "circle", fill_col = fill_col, border_col = border_col)
#
# naes_rich1 <- format_pict(path_ori = path_all_picts, path_final = path_naesrich, name_ori = paste0(corner_pict$naes_rich[1], ".png"), name_final = "naes_rich1.pdf", point_type = "triangle", fill_col = fill_col, border_col = border_col)
# naes_rich2 <- format_pict(path_ori = path_all_picts, path_final = path_naesrich, name_ori = paste0(corner_pict$naes_rich[2], ".png"), name_final = "naes_rich2.pdf", point_type = "triangle", fill_col = fill_col, border_col = border_col)
# naes_rich3 <- format_pict(path_ori = path_all_picts, path_final = path_naesrich, name_ori = paste0(corner_pict$naes_rich[3], ".png"), name_final = "naes_rich3.pdf", point_type = "triangle", fill_col = fill_col, border_col = border_col)
# naes_rich4 <- format_pict(path_ori = path_all_picts, path_final = path_naesrich, name_ori = paste0(corner_pict$naes_rich[4], ".png"), name_final = "naes_rich4.pdf", point_type = "triangle", fill_col = fill_col, border_col = border_col)
#
# aes_rich1  <- format_pict(path_ori = path_all_picts, path_final = path_aesrich, name_ori = paste0(corner_pict$aes_rich[1], ".png"), name_final = "aes_rich1.pdf", point_type = "diamond", fill_col = fill_col, border_col = border_col)
# aes_rich2  <- format_pict(path_ori = path_all_picts, path_final = path_aesrich, name_ori = paste0(corner_pict$aes_rich[2], ".png"), name_final = "aes_rich2.pdf", point_type = "diamond", fill_col = fill_col, border_col = border_col)
# aes_rich3  <- format_pict(path_ori = path_all_picts, path_final = path_aesrich, name_ori = paste0(corner_pict$aes_rich[3], ".png"), name_final = "aes_rich3.pdf", point_type = "diamond", fill_col = fill_col, border_col = border_col)
# aes_rich4  <- format_pict(path_ori = path_all_picts, path_final = path_aesrich, name_ori = paste0(corner_pict$aes_rich[4], ".png"), name_final = "aes_rich4.pdf", point_type = "diamond", fill_col = fill_col, border_col = border_col)

# here for the example, load a fake picture
fake_pict <- format_pict(path_ori = hh("data/"), path_final = hh("output/"), name_ori = paste0("fake_picture", ".png"),
                         name_final = "11_fake_picture.pdf", point_type = "diamond", fill_col = fill_col,
...
# +
  ...
  #                     ggplot2::aes(x = df$eco_rank[which(df$eco_rank %in% aes_p$eco_rank)], y = df$esth_rank[which(df$eco_rank %in% aes_p$eco_rank)]) , shape = 22, bg = fill_col, col = border_col, size = 4) +
...
ggplot2::ggsave(plot = center_plot, filename = hh("output", "11_center_plot.pdf"), width = 20, height = 20, units = "cm")
...
final_plot <- magick::image_composite(frame_plot, magick::image_scale(center_plot, "950x950"), offset = "+300+254")
#
pdf(file = hh("output", "11_Figure5.pdf"), width = 11, height = 11, family = "sans")
par(mar = c(0,0,0,0))
plot(final_plot)
# eco arrow
shape::Arrows(x0 = 335, y0 = 315, x1 = 1210, y1 = 315, arr.length = 0.15, arr.width = 0.15, arr.type = "triangle")
# esth arrow
shape::Arrows(x0 = 320, y0 = 325, x1 = 320, y1 = 1198, arr.length = 0.15, arr.width = 0.15, arr.type = "triangle")
# axis title
...
rm(table, border_col, center_plot, fake_pict, fill_col, final_plot, frame_plot, pal, xlim, ylim, format_pict)
