...
# unique station
dupl <- as.character(table_station$station[which(duplicated(table_station$station) &
                                                   duplicated(table_station$depth))])
dupl <- unique(dupl)
...
col_mean <- colnames(table)[-which(colnames(table) %in% c("quadrat_code", "site", "station",
...
table_station$esth_score       <- 
  as.numeric(levels(table_station$esth_score))[table_station$esth_score]
table_station$qTD              <- 
  as.numeric(levels(table_station$qTD))[table_station$qTD]
table_station$SES_qPD          <- 
  as.numeric(levels(table_station$SES_qPD))[table_station$SES_qPD]
table_station$SES_qFD          <-
  as.numeric(levels(table_station$SES_qFD))[table_station$SES_qFD]
table_station$Exploitation     <- 
  as.numeric(levels(table_station$Exploitation))[table_station$Exploitation]
table_station$Anthropization   <- 
  as.numeric(levels(table_station$Anthropization))[table_station$Anthropization]
table_station$substrate_recouv <-
  as.numeric(levels(table_station$substrate_recouv))[table_station$substrate_recouv]
...
data_station$esth_class         <- cut(data_station$esth_score, 
                                       breaks = c(min(data_station$esth_score)-1, c1, c2, c3, c4,
                                                  max(data_station$esth_score)+1))
...
col <- cbind.data.frame(class = levels(data_station$esth_class), 
                        col = viridis::viridis(n = 5, direction = 1, option = pal))
for (i in 1: nrow(data_station)){
  for(j in 1 : nrow(col)){
    if(as.character(data_station$esth_class[i]) == as.character(col$class[j])){
      data_station$col[i] <- levels(col$col)[col$col][j]} # eo if
  } # eo for j
} # eo for i
...
data_station$eco_class          <- cut(data_station$eco_rank, 
                                       breaks = c(min(data_station$eco_rank)-1, s1, s2, s3, s4,
                                                  max(data_station$eco_rank)+1))
levels(data_station$eco_class ) <- c(paste0("≤",s1), paste0("(",s1,";",s2,"]"), 
                                     paste0("(",s2,";",s3,"]"), paste0("(",s3,";",s4,"]"),
                                     paste0(">",s4))
# Attribute a size to each class of eco rank. The lower the eco rank, the higher the eco value and
# the bigger the point
data_station$size[order(data_station$eco_class)] <- c(5,4,3,2,1)[as.numeric(data_station$eco_class[
  order(data_station$eco_class)])]
...
# Plot distrib ----
model                  <- lm(
  formula = as.numeric(as.character(esth_rank)) ~ as.numeric(as.character(eco_rank)), 
  data = data_station)
...
# marginal density
q <- ggExtra::ggMarginal(p, type = "density", margins = "both", color = "lightgray",
                         fill = "gray85")

ggplot2::ggsave(q, filename = hh("output", "12_esth_eco_density.png"), bg = "transparent",
                width = 13, height = 13)
...
# TOP
# create empty plot
plot(1, type = "n", xaxt = "n", yaxt = "n", xlim = xlims, ylim = ylims_lat, xlab = "", ylab = "",
     bty = "n")
# add raster background
plot(cont_bound_rast, add = TRUE, col = "gray85", legend = FALSE) 
# longitudes
axis(side = 3, las = 1, at = c(2, 3, 4, 5, 6, 7), cex.lab = 1, lwd = -1, lwd.ticks = 2,
     col.ticks = "gray", labels = c("2°E", "3°E", "4°E", "5°E", "6°E", "7°E"))
# latitudes
axis(side = 2, las = 1, at = c(42, 42.5, 43, 43.5, 44, 44.5), cex.lab = 1, lwd = -1, lwd.ticks = 2,
     col.ticks = "gray", labels = c("42°N", "42.5°N", "43°N", "43.5°N", "44°N", "44.5°N" ))
# plot sites
points(data_cont$longitude, data_cont$latitude, pch = 21, 
       col = scales::alpha(colour = "black", alpha = 0.6),
       bg = scales::alpha(colour = "gray", alpha = 0.6), cex = 1)
# plot Marseille
...
# create empty plot
plot(1, type = "n", bty = 'n', xaxt = "n", yaxt = 'n', ylab = "Depth(m)", xlab = "", xlim = xlims,
     ylim = ylims_depth)
# add depth axis
axis(side = 2, las = 1, at = c(20, 30, 40, 50, 60, 70, 80), cex.lab = 1, lwd = -1, lwd.ticks = 2,
     col.ticks = "gray", labels = c("20", "30", "40", "50", "60", "70", "80"))
...
# station points
points(x = data_cont$longitude, y = data_cont$profondeur, 
       col = scales::alpha(colour = data_cont$col, alpha = 0.7),
       pch = 20, cex = data_cont$size*0.7)
dev.off()
...
sites <- unique(cbind.data.frame(site = data_corse$site, longitude = data_corse$longitude,
                                 latitude  = data_corse$latitude))
x0    <- data_corse$longitude
y0    <- data_corse$latitude

# polar coordinate of the sites
polar_sites   <- cbind.data.frame(site = data_corse$site, useful::cart2pol((
  data_corse$longitude - as.numeric(centroide[,"lon"])), 
...
X     <- cbind(as.vector(rbind(data_corse$longitude, centroide[,"lon"])), 
               as.vector(rbind(data_corse$latitude, centroide[,"lat"])))
n     <- nrow(X)
...
pt_40r   <- cbind.data.frame(site = polar_sites$site, theta = polar_sites$theta, 
                             r = rep(radius[1], nrow(polar_sites)))
...
# pt 40v : same but with the segment comes from the virtual site. 
# Prevent overlapping of points and labels
pt_40v <- cbind.data.frame(site = polar_sites$site, theta = pt_40r$theta, r = pt_40r$r)
change <- c("St-Florent", "Canari", "Centuri", "Scandola-1","Sanguinaires", "BonifacioS1",
            "Lavezzi", "Tarco", "Tarco2")
adjust <- c(-0.05, -0.02 , 0.02, 0.02, -0.02, -0.05, 0.05, 0, 0.05)
for (i in 1:length(change)){ 
  pt_40v$theta[which(pt_40v$site == change[i])] <- 
    pt_40v$theta[which(pt_40v$site == change[i])] + adjust[i]}
...
# pt 90 : projection of pt_40v on the last circle
  pt_90   <- cbind.data.frame(site = polar_sites$site, theta = pt_40v$theta,
                              r = rep(radius[length(radius)], nrow(polar_sites)))
...
polar_corse           <- cbind.data.frame(site    = data_corse$site, 
                                          station = data_corse$station,
                                          depth   = data_corse$profondeur, 
                                          esth    = data_corse$esth_score, 
                                          ecoval  = data_corse$eco_rank, 
                                          col     = data_corse$col, 
                                          cex     = data_corse$size)
polar_corse$theta_sta <- pt_90$theta
polar_corse$r_sta     <- r + ((polar_corse$depth - 30) / 100)
polar_corse$x_sta     <- as.numeric(centroide[,"lon"]) + useful::pol2cart(polar_corse$r_sta,
                                                                          polar_corse$theta_sta)$x
polar_corse$y_sta     <- as.numeric(centroide[,"lat"]) + useful::pol2cart(polar_corse$r_sta,
                                                                          polar_corse$theta_sta)$y

# coordinates for the scale
x0     <- as.numeric(centroide[,"lon"]) + useful::pol2cart(r = radius[1], theta = 180 )$x 
y0     <- as.numeric(centroide[,"lat"])
x1     <- as.numeric(centroide[,"lon"]) + useful::pol2cart(r = radius[length(radius)],
                                                           theta = 180 )$x
y1     <- y0
...
par(mar = c(0,3,2,3))
# empty plot
plot(1, type = "n", xaxt = "n", yaxt = "n", xlim = xlims, ylim = ylims, xlab = "", ylab = "",
     bty = 'n')
...
segments(x0 = data_corse$longitude, y0 = data_corse$latitude, x1 = pt_40r$x, y1 = pt_40r$y,
         lty = "dotted", col = "lightgray", lwd = 1.2, xpd = TRUE)
# segments from pt_40v to pt_90 
segments(x0 = pt_40v$x, y0 = pt_40v$y, x1 = pt_90$x, y1 = pt_90$y, lty = "dotted",
         col = "lightgray", lwd = 1.2)
for (i in 1: length(radius)){plotCircle(as.numeric(centroide[,"lon"]),
                                        as.numeric(centroide[, "lat"]), radius[i], lty = 3,
                                        col = "lightgray", lwd = 1.2)} # eo for i
# white polygon to hide east side circles
polygon(x = c(9.7, 10.8, 10.8, 9.7), y = c(42.9, 44.5, 40.3, 41.5), col = "white", 
        border = "white")
# longitude axis
axis(side = 3, las = 1, cex = 1, lwd = -1, lwd.ticks = 2, col.ticks = "gray", at = c(8, 9, 10),
     labels = c("8°E", "9°E", "10°E"))
# latitude axis
axis(side = 4, las = 1, pos = 10.2, cex = 01, lwd = -1, lwd.ticks = 2, col.ticks = "gray", 
     at = c( 41, 41.5, 42, 42.5, 43, 43.5), 
     labels = c("41°N", "41.5°N", "42°N", "42.5°N", "43°N","43.5°N"))
# stations
points(x = polar_corse$x_sta, y = polar_corse$y_sta, pch = 20, 
       col = scales::alpha(polar_corse$col, 0.7), cex = polar_corse$cex*0.7, xpd = TRUE)
# depth axis
arrows(x0 = 8.09, y0 = 42, x1 = 7.58, y1 = 42, col = "black", length = 0.08)
# depth labels
text(x = c(8.1, 7.9, 7.7), y = 42, labels = as.character(depths)[c(1,3,5)], pos = 1, cex = 0.6,
     col = "black")
# title of depth axis
text(x = 7.85, y = 42, labels = "Depth (m)", pos = 3, cex = 0.7, col = "black", xpd = TRUE)
# scale
maps::map.scale(x = 7.8, y = 40.6, relwidth = 0.15, metric = TRUE, ratio = FALSE, cex = 0.5,
                xpd = TRUE)
dev.off()
...
par(mar = c(0,0,0,0))
# purple 
plot(x = xs, y = ys, xlim = c(0,10), ylim = c(0,10), axes = FALSE, xlab = "", ylab = "", pch = 19,
     cex = leg_size$cex + 3, col = unique(data_cont$col[order(data_cont$esth_score)])[1])
# deep blue
points(x = xs, y = ys+1.1, pch = 19, cex = leg_size$cex + 3, 
       col = unique(data_cont$col[order(data_cont$esth_score)])[2])
# turquoise
points(x = xs, y = ys+2.2, pch = 19, cex = leg_size$cex +3,
       col = unique(data_cont$col[order(data_cont$esth_score)])[3])
# green
points(x = xs, y = ys+3.3, pch = 19, cex = leg_size$cex+3,
       col = unique(data_cont$col[order(data_cont$esth_score)])[4])
# yellow
points(x = xs, y = ys+4.4, pch = 19, cex = leg_size$cex+3,
       col = unique(data_cont$col[order(data_cont$esth_score)])[5])
# arrows
arrows(x0 = xs[1]-0.25,         y0 = ys[1]+5.3, x1 = xs[length(xs)]+0.45, y1 = ys[1]+5.3, code = 2,
       length = 0.1, lwd = 2)
arrows(x0 = xs[length(xs)]+0.7, y0 = ys[1]-0.7, x1 = xs[length(xs)]+0.7,  y1 = ys[1]+5,   code = 2,
       length = 0.1, lwd = 2)
# labels
graphics::text (x = 4.5,                y = ys[1]+5.8, labels = "Ecological value", cex = 2.5)
graphics::text (x = xs[length(xs)]+1.2, y = 4,         labels = "Aesthetic value",  cex = 2.5,
                srt = 270)
...
# Add the legend
compo_leg <- magick::image_composite(both, magick::image_scale(leg_transp, "1100x1100"), 
                                     offset = "+1750+1100")
# Add the density plot
full_compo <- magick::image_composite(compo_leg, magick::image_scale(dens, "800x800"), 
                                      offset = "+1950+170")
...
rm(list=ls(all=TRUE)) 
file.remove(hh("output", "12_corsica_map.pdf"), hh("output", "12_esth_eco_density.png"),
            hh("output", "12_legend.png"), hh("output", "12_mainland_map.pdf"))
# ----
